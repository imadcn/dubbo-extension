...
public class ConsumerLogFilter implements Filter {
	...
	private String getRpcLog(String className, String methodName, String inputParams, String rspResult, long startTime, long endTime) {
		...
		return String.format("[DUBBO-Consumer] %s->%s - %s|%s|IN:%s|OUT:%s|[start:%s, end:%s, cost:%dms]", localAddress, remoteAddress, className, methodName, inputParams, rspResult, startTimeStr, endTimeStr, cost);
	}
}
